/* ══════════════════════════════════════════
   낱글자 팡팡! — 한글 단어 버블슈터 (UI/UX 100% 완성본)
   ══════════════════════════════════════════ */
...
const PALETTE = [
  ['#8fc98a','#4f8a52'],['#e8a6c2','#c96f95'],['#8fb8d8','#4f7fa8'],
  ['#e8c27a','#b8863f'],['#e0dcd0','#b0a890'],['#b39ad6','#7c5ba8']
];
const NCOL=6;
...
const G={
  ...
  cur:null,queue:[],fly:null,aim:null,dragging:false,
...
function buildStage(){
  ...
  const others=shuffle(CATS.filter(c=>c!==G.goal)).slice(0,2).flatMap(c=>shuffle(DICT_BY_CAT[c]).slice(0,3));
...
function sprinkleSpecials(){
  ...
  for(let r=0;r<G.grid.length;r++)
    for(let c=0;c<cellsIn(r);c++) if(at(c,r) && !G.grid[r][c].special) all.push([c,r]);
  shuffle(all);
  ...
  for(const ax of AXES){
    ...
    for(let len=Math.min(MAXW,str.length);len>=2;len--){
      for(let st=Math.max(0,idx-len+1);st<=Math.min(idx,str.length-len);st++){
        ...
        else{ const rw=[...w].reverse().join(''); if(rw!==w && DICT.has(rw)){ cands.push({word:rw,cells:cells.slice(st,st+len),cat:DICT.get(rw)}); } }
      }
    }
  }
...
function completionsFor(s){
  const res=[]; for(const [c,r] of openCells()){ const w=findWord(c,r,s); if(w)res.push({c,r,...w}); } return res;
}
let _recentSyl=[];
function _notRecent(list){ const fresh=list.filter(s=>!_recentSyl.includes(s)); return fresh.length?fresh:list; }
function newCur(){ return {s:smartSyllable(), col:randCol()}; }
function smartSyllable(){
  let chosen=null;
  if(Math.random()<0.78){
    const targetC=[]; const anyC=[];
    for(const s of G.pool){
      const hit=completionsFor(s); if(!hit.length) continue;
      anyC.push(s); if(G.mode==='theme' && hit.some(h=>h.word in G.done && !G.done[h.word])) targetC.push(s);
    }
    let pool2;
    if(G.mode==='theme' && targetC.length){ pool2 = (Math.random()<0.66) ? targetC : (anyC.length?anyC:targetC); }
    else{ pool2 = anyC; }
    if(pool2.length) chosen=pick(_notRecent(pool2));
  }
  if(!chosen) chosen=pick(_notRecent(G.pool));
  _recentSyl.push(chosen); if(_recentSyl.length>3) _recentSyl.shift(); return chosen;
}
function ensureRow(r){ while(G.grid.length<=r) G.grid.push(new Array(cellsIn(G.grid.length)).fill(null)); }

function shoot(angle){
  if(G.fly||G.locked)return;
  const sp=R*0.62; const item=G.activeItem;
  if(item==='bomb') G.bombs--; if(item==='rainbow') G.rainbows--;
  G.activeItem=null; const p0=pipePos();
...
function bestSyllableAt(c,r){
  let best=null;
  for(const s of G.pool){ const hit=findWord(c,r,s); if(hit && (!best || hit.word.length>best.word.length)) best={s,...hit}; }
  return best;
}
function explodeAt(c,r){
  ...
  setTimeout(()=>{
    ...
    for(const [cc,rr] of cells){
      ...
      const col=colorOf(G.grid[rr][cc])[0]; burst(cx(cc,rr),cy(rr),col); addWave(cx(cc,rr),cy(rr),col,R*3); G.grid[rr][cc]=null; n++;
    }
    addShake(10); flash(0.22); G.score += n*80; G.combo=0; G.dryShots=0;
    toast('펑! +'+(n*80)); dropFloaters(); G.locked=false; checkState(); syncUI();
  },260);
}
function settle(f){
  ...
  if(!best){
    ensureRow(0);
    for(let c=0;c<cellsIn(0);c++){ if(G.grid[0][c])continue; const d=(f.x-cx(c,0))**2+(f.y-cy(0))**2; if(d<bd){bd=d;best=[c,0];} }
  }
  ...
  let s=f.s; if(f.item==='rainbow'){ const b=bestSyllableAt(c,r); if(b) s=b.s; }
  G.grid[r][c]={s,col:f.col,born:performance.now()};
  addShake(1.5); addWave(cx(c,r),cy(r),colorOf(G.grid[r][c])[0],R*1.4); resolve(c,r);
}
function wordScore(len,combo){ const base=[0,0,200,600,1600,3200][Math.min(len,5)]||3200; return base*Math.max(1,combo); }
function findWordAt(c0,r0){
  const b0=at(c0,r0); if(!b0)return null; let best=null;
  const dfs=(c,r,str,path,visited)=>{
    if(str.length>=2){
      const rev=[...str].reverse().join(''); let hit=DICT.has(str)?str:(DICT.has(rev)?rev:null);
      if(hit && (!best||hit.length>best.word.length)) best={word:hit,cells:path.slice(),cat:DICT.get(hit)};
    }
    if(str.length>=MAXW)return;
    for(const [nc,nr] of nbrs(c,r)){
      const k=nc+','+nr; if(visited.has(k))continue; const b=at(nc,nr); if(!b)continue;
      visited.add(k); path.push([nc,nr]); dfs(nc,nr,str+b.s,path,visited); path.pop(); visited.delete(k);
    }
  };
...
function floodMatch(c0,r0,key){
  ...
  while(stack.length){
    const [c,r]=stack.pop(); const k=c+','+r; if(seen.has(k))continue; seen.add(k);
    const b=at(c,r); if(!b)continue; const v=key==='col'?b.col:b.s; if(v!==target)continue; out.push([c,r]);
...
function resolve(c,r){
  ...
  if(word){
    ...
    if(word.word in G.done && !G.done[word.word]) {
      G.done[word.word]=true; const tBar = document.getElementById('targetBar');
      if(tBar) {
        const chips = tBar.querySelectorAll('.tchip');
        chips.forEach(chip => {
          if(chip.textContent.replace('✓ ','').trim() === word.word) {
            chip.style.transform = 'scale(1.25)'; chip.style.boxShadow = '0 0 20px #7ba05b'; chip.style.zIndex = '10';
            setTimeout(() => { chip.style.transform = ''; chip.style.boxShadow = ''; chip.style.zIndex = ''; }, 400);
          }
        });
      }
    }
    const combo=Math.min(G.combo,5); const pts=wordScore(word.word.length, combo); G.score+=pts;
    ...
    G.locked=true; clearCells(word.cells);
    word.cells.forEach(([cc,rr],i)=>{ setTimeout(()=>{ addWave(cx(cc,rr),cy(rr),'#ffe08c',R*3); if(i>0)SFX.pop(); },i*70); });
    if(word.word.length>=4){
      for(let i=0;i<20;i++){
        const p = getParticle();
        if(p) { const ang=Math.random()*Math.PI*2, sp=1+Math.random()*4; p.active=true; p.x=W/2; p.y=H*0.4; p.vx=Math.cos(ang)*sp; p.vy=Math.sin(ang)*sp; p.life=1.4; p.col='#ffe08c'; p.r=2+Math.random()*3; }
      }
    ...
    setTimeout(()=>{
      ...
      for(const [cc,rr] of word.cells){
        if(!G.grid[rr]||!G.grid[rr][cc])continue; const sp=G.grid[rr][cc].special; if(sp==='gold')goldHit++; if(sp==='bomb')bombCells.push([cc,rr]); burst(cx(cc,rr),cy(rr),'#ffe08c'); G.grid[rr][cc]=null;
      }
      let chain=0;
      for(const [wc,wr] of word.cells) for(const [nc,nr] of nbrs(wc,wr)) if(G.grid[nr]&&G.grid[nr][nc]){ burst(cx(nc,nr),cy(nr),colorOf(G.grid[nr][nc])[0]); G.grid[nr][nc]=null; chain++; }
      G.score+=chain*30;
  ...
  const colGroup=floodMatch(c,r,'col'); const sylGroup=floodMatch(c,r,'syl'); let group=[], mtype='';
  ...
  if(group.length>=3){
    G.combo++; G.dryShots=0; const combo=Math.min(G.combo,5); const typeMul=mtype==='syl'?1.5:1; let pts=Math.round(group.length*12*combo*typeMul);
    ...
    G.locked=true; clearCells(group);
    setTimeout(()=>{ for(const [cc,rr] of group){ if(!G.grid[rr]||!G.grid[rr][cc])continue; burst(cx(cc,rr),cy(rr),colorOf(G.grid[rr][cc])[0]); addWave(cx(cc,rr),cy(rr),colorOf(G.grid[rr][cc])[0]); G.grid[rr][cc]=null; } dropFloaters(); G.locked=false; checkState(); syncUI(); },300);
    syncUI(); return;
  }
  ...
  if(G.dryShots>=5){G.dryShots=0;addRow();} checkState(); syncUI();
}
...
function addRow(){
  SFX.rowAdd(); G.parity^=1; const row=[];
  for(let c=0;c<cellsIn(0);c++){
    const avoid=c>0?row[c-1].s:null; let cand=G.pool.filter(x=>x!==avoid); if(!cand.length)cand=[...G.pool];
    let mn=Infinity; for(const x of cand) mn=Math.min(mn,_fillCount[x]||0); const least=cand.filter(x=>(_fillCount[x]||0)<=mn+1);
    const ch=pick(least.length?least:cand); _fillCount[ch]=(_fillCount[ch]||0)+1; row.push({s:ch,col:randCol()});
  }
  G.grid.unshift(row); toast('새 줄이 내려왔어요!');
}
function checkState(){
  let count=0,lowest=-1;
  for(let r=0;r<G.grid.length;r++) for(let c=0;c<cellsIn(r);c++) if(at(c,r)){count++;lowest=Math.max(lowest,r);}
  if(count===0){win();return;}
  if(G.mode==='theme'){ const allDone=G.targets.length && G.targets.every(w=>G.done[w]); if(allDone){win();return;} }
  else if(G.mode==='free'){ if(G.wordsCompleted>=G.freeGoal){win();return;} }
...
function burst(x,y,col){
  for(let i=0;i<14;i++){ const p = getParticle(); if(!p) continue; const a=Math.random()*Math.PI*2,sp=1+Math.random()*4.5; p.active=true; p.x=x; p.y=y; p.vx=Math.cos(a)*sp; p.vy=Math.sin(a)*sp-1; p.life=1; p.col=col; p.r=2+Math.random()*4; } blip();
}
...
function tone(freq0,freq1,dur,gain,type,delay){
  if(!soundOn())return; const ax=getActx(); if(!ax)return;
  try{ const t0=ax.currentTime+(delay||0); const o=ax.createOscillator(),g=ax.createGain(); o.type=type||'triangle'; o.frequency.setValueAtTime(freq0,t0); if(freq1) o.frequency.exponentialRampToValueAtTime(freq1,t0+dur*0.55); g.gain.setValueAtTime(0.0001,t0); g.gain.exponentialRampToValueAtTime(gain,t0+0.015); g.gain.exponentialRampToValueAtTime(0.0001,t0+dur); o.connect(g); g.connect(ax.destination); o.start(t0); o.stop(t0+dur+0.02); }catch(e){}
}
const SFX={
  pop(){ tone(520+Math.random()*260,880,0.17,0.07,'triangle'); },
  click(){ tone(700,900,0.06,0.05,'square'); },
  wordComplete(combo,bonus){ const base=440+Math.min(combo,5)*40; const ratios=bonus?[1,1.26,1.5,2]:[1,1.26,1.5]; ratios.forEach((r,i)=>tone(base*r,base*r*1.15,0.22,0.09,'triangle',i*0.045)); },
  miss(){ tone(260,180,0.14,0.045,'sine'); },
  rowAdd(){ tone(180,120,0.3,0.06,'sawtooth'); },
  stageClear(){ [0,1,2,3].forEach(i=>tone(523.25*Math.pow(2,i/12*4), null, 0.28,0.08,'triangle',i*0.11)); },
...
const ASSETS={};
function loadAssets(){ return Promise.all(Object.entries(ASSET_SRC).map(([k,src])=>new Promise(res=>{ const i=new Image(); i.onload=()=>{ASSETS[k]=i;res();}; i.onerror=()=>{console.warn('asset error:',k);res();}; i.src=src; }))); }
...
function drawBubbleRaw(x,y,r,s,col,glow,special){
  ...
  const rr=r*.94; const gemImg = (special==='gold') ? ASSETS.gem3 : (special==='bomb' ? ASSETS.gem1 : ASSETS['gem'+((col||0)%6)]);
  if(gemImg){ ctx.save(); ctx.beginPath(); ctx.arc(x,y,rr,0,7); ctx.clip(); ctx.drawImage(gemImg, x-rr, y-rr, rr*2, rr*2); if(special==='gold'){ ctx.fillStyle='rgba(255,224,140,.28)'; ctx.fillRect(x-rr,y-rr,rr*2,rr*2); } ctx.restore(); }
  else{ ctx.save(); ctx.beginPath(); ctx.arc(x,y,rr,0,7); ctx.clip(); const g=ctx.createRadialGradient(x-rr*.32,y-rr*.38,rr*.12, x,y,rr*1.15); g.addColorStop(0, lighten(c1,18)); g.addColorStop(.45, c1); g.addColorStop(1, c2); ctx.fillStyle=g; ctx.fillRect(x-rr,y-rr,rr*2,rr*2); ctx.restore(); }
  ctx.save(); ctx.beginPath(); ctx.arc(x,y,rr,0,7); ctx.strokeStyle= glow ? '#fff0c0' : '#c9a04a'; ctx.lineWidth=Math.max(1.2,r*.055); ctx.globalAlpha=.85; ctx.stroke(); ctx.restore();
  ...
  ctx.save(); ctx.font=`800 ${r*1.0}px 'Pretendard', sans-serif`; ctx.textAlign='center';ctx.textBaseline='middle'; const ty=y+r*.06; ctx.shadowColor='rgba(40,24,10,.55)'; ctx.shadowBlur=r*.14; ctx.shadowOffsetY=r*.07; ctx.fillStyle='#fffaf0'; ctx.fillText(s,x,ty); ctx.shadowColor='transparent'; ctx.shadowBlur=0; ctx.shadowOffsetY=0;
  if(special==='gold'){ ctx.strokeStyle='#fff3b0'; ctx.lineWidth=r*.09; ctx.shadowColor='#ffe08c'; ctx.shadowBlur=r*.6; ctx.beginPath(); ctx.arc(x,y,rr,0,7); ctx.stroke(); ctx.shadowBlur=0; ctx.fillStyle='#fff8d8'; [[0.5,-0.7],[-0.6,0.4],[0.7,0.5],[-0.4,-0.5]].forEach(([dx,dy],i)=>{ const s2=r*0.10*(0.7+0.5*Math.sin(performance.now()/200+i)); ctx.beginPath(); ctx.arc(x+dx*r*.7,y+dy*r*.7,s2,0,7); ctx.fill(); }); }
  else if(special==='bomb'){ ctx.font=`${r*.55}px sans-serif`; ctx.textAlign='center'; ctx.textBaseline='middle'; ctx.fillText('💣', x+r*.5, y-r*.5); } ctx.restore();
}
function lighten(hex,amt){ const n=parseInt(hex.slice(1),16); let r=(n>>16)&255, g=(n>>8)&255, b=n&255; return `rgb(${Math.min(255,r+amt*2)|0},${Math.min(255,g+amt*2)|0},${Math.min(255,b+amt*2)|0})`; }
const SPR=new Map(); let FONTS_READY=false;
function sprite(s,r,glow,col){
  const key=s+'|'+r+'|'+(glow?1:0)+'|'+(col||0); let c=SPR.get(key); if(c)return c;
  const pad=Math.ceil(r*(glow?2.0:1.55)), size=pad*2; c=document.createElement('canvas'); c.width=Math.ceil(size*DPR); c.height=Math.ceil(size*DPR); const g2=c.getContext('2d'); g2.setTransform(DPR,0,0,DPR,0,0); const keep=ctx; ctx=g2; drawBubbleRaw(pad,pad,r,s,col,glow); ctx=keep; c._pad=pad; if(FONTS_READY){ if(SPR.size>320)SPR.clear(); SPR.set(key,c); } return c;
}
...
let BOARDLAYER=null;
function drawBoard(){
  if(!BOARDLAYER){ const c=document.createElement('canvas'); c.width=Math.ceil(W*DPR); c.height=Math.ceil(H*DPR); const g2=c.getContext('2d'); g2.setTransform(DPR,0,0,DPR,0,0); const keep=ctx; ctx=g2; const x=BX-FRAME,y=BY-FRAME,w=BW+FRAME*2,h=BH+FRAME*2; ctx.save(); roundRect(x,y,w,h,24); ctx.fillStyle='rgba(20,12,6,.16)';ctx.fill(); ctx.restore(); ctx=keep; BOARDLAYER=c; }
  ctx.drawImage(BOARDLAYER,0,0,W,H);
}
const CANNON_W=4.8, CANNON_OFF=0.0; const PIPE_X_RATIO=0.52, PIPE_Y_RATIO=-0.05;
function pipePos(){ const cx0=W/2, sy=G.shooterY; const img=ASSETS.cannon; if(!img) return {x:cx0,y:sy}; const w=R*CANNON_W, h=w*(img.height/img.width); const ix=cx0-w/2, iy=sy+R*CANNON_OFF; return {x: ix+w*PIPE_X_RATIO, y: iy+h*PIPE_Y_RATIO}; }
function drawShooter(now){
  const img=ASSETS.cannon, sy=G.shooterY, cx0=W/2; const p=pipePos(); const pipeX=p.x, pipeY=p.y;
  if(img){ const w=R*CANNON_W, h=w*(img.height/img.width); ctx.drawImage(img, cx0-w/2, sy+R*CANNON_OFF, w, h); }
  if(!G.fly&&G.cur){ const bob=Math.sin(now/420)*R*.05; bubble(pipeX,pipeY+bob,R*.94,G.cur.s,G.cur.col); if(G.activeItem){ const icon=G.activeItem==='bomb'?'💣':'🌈'; ctx.save(); ctx.font=`${R*.62}px sans-serif`; ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText(icon, pipeX+R*0.78, pipeY+bob-R*0.78); ctx.restore(); } }
}
function drawQueue(){
  if(!G.queue.length)return; const x=W/2 + R*2.8, y=G.shooterY + R*0.3, r=R*.82;
  ...
  neon('#c8962f',6,2,()=>{ctx.beginPath();ctx.arc(x,y,r*1.12,0,7);ctx.stroke();}); bubble(x,y,r*.92,G.queue[0].s,G.queue[0].col);
}
function trajectory(angle){
  let x=W/2,y=G.shooterY,vx=Math.cos(angle)*R*.62,vy=Math.sin(angle)*R*.62; const pts=[];
  for(let i=0;i<200;i++){ x+=vx;y+=vy; if(y<BY+BH){ if(x<BX+R){x=BX+R;vx*=-1;} if(x>BX+BW-R){x=BX+BW-R;vx*=-1;} } if(y<=BY+R)break; if(y<BY+BH&&hitsBubble(x,y))break; if(i%4===0)pts.push([x,y]); } return pts;
}
function draw(now){
  ctx.clearRect(0,0,W,H); ctx.save(); if(G.shake>0.3){ const dx=(Math.random()-0.5)*G.shake, dy=(Math.random()-0.5)*G.shake; ctx.translate(dx,dy); }
  drawBoard();
  if(G.aim!=null&&!G.fly&&!G.locked){
    if(G.trajA!==G.aim){G.trajA=G.aim;G.trajPts=trajectory(G.aim);} const pts=G.trajPts;
    ctx.save();ctx.fillStyle='#f0d896';ctx.shadowColor='#c8962f';ctx.shadowBlur=8; pts.forEach((p,i)=>{ctx.beginPath();ctx.arc(p[0],p[1],R*.13*(1-i/pts.length*.5),0,7);ctx.fill();});
    if(pts.length>2){ const a=pts[pts.length-1],b2=pts[pts.length-2]; const ang=Math.atan2(a[1]-b2[1],a[0]-b2[0]); ctx.translate(a[0],a[1]);ctx.rotate(ang+Math.PI/2); ctx.beginPath(); ctx.moveTo(0,-R*.42);ctx.lineTo(R*.30,R*.18);ctx.lineTo(-R*.30,R*.18); ctx.closePath();ctx.fill(); } ctx.restore();
  }
  if(G.hintCells){
    ctx.save(); const pulse=.5+.5*Math.sin(now/180); ctx.strokeStyle=`rgba(255,232,140,${.5+pulse*.5})`;ctx.shadowColor='#ffe08c';ctx.shadowBlur=14; ctx.lineWidth=4;ctx.setLineDash([7,7]);
    for(const [c,r] of G.hintCells){ctx.beginPath();ctx.arc(cx(c,r),cy(r),R*1.05,0,7);ctx.stroke();} ctx.restore();
  }
  for(let r=0;r<G.grid.length;r++) for(let c=0;c<cellsIn(r);c++){
    const b=at(c,r); if(!b)continue; let rr=R*.94; if(b.born){const t=Math.min(1,(now-b.born)/220);rr=R*.94*(.62+.38*t+.12*Math.sin(t*Math.PI));}
    let bx=cx(c,r), by=cy(r); if(b.nope){ const dt=now-b.nope; if(dt<400){ bx+=Math.sin(dt/28)*Math.max(0,4-dt/100); } else b.nope=0; } bubble(bx,by,rr,b.s,b.col,!!b.glow,b.special);
  }
  if(G.fly)bubble(G.fly.x,G.fly.y,R*.94,G.fly.s,G.fly.col);
  for(let i=0; i<MAX_PARTICLES; i++){ const p = PARTICLE_POOL[i]; if(p.active){ ctx.globalAlpha=Math.max(0,p.life); ctx.beginPath();ctx.arc(p.x,p.y,p.r,0,7); ctx.fillStyle=p.col;ctx.shadowColor=p.col;ctx.shadowBlur=12; ctx.fill();ctx.shadowBlur=0; } } ctx.globalAlpha=1;
  drawShooter(now); drawQueue();
  if(G.banner){
    const b=G.banner, t=1-b.life/(b.big?1.6:1); const pop=t<.18?(t/.18):1; const bigMul=b.big?1.5:1; const fs=Math.min(R*2.0*bigMul,BW/(b.text.length+0.45)*bigMul)*(0.72+0.28*pop)*(1+(1-b.life)*0.06); const col=b.bonus?'#ffd86f':'#ff8fdc';
    ctx.save(); ctx.globalAlpha=Math.min(1,b.life*2.2); ctx.translate(W/2,BY+R*1.25-(1-b.life)*R*.8); if(b.big){ const rot=Math.sin(performance.now()/90)*0.04; ctx.rotate(rot); }
    ctx.font=`800 ${fs}px 'Pretendard', sans-serif`; ctx.textAlign='center';ctx.textBaseline='middle'; ctx.shadowColor=col;ctx.shadowBlur=fs*(b.big?0.8:0.55); ctx.fillStyle=col;ctx.fillText(b.text,0,0);ctx.fillText(b.text,0,0); if(b.big) ctx.fillText(b.text,0,0); ctx.shadowBlur=0; ctx.fillStyle='#ffffff';ctx.fillText(b.text,0,0); ctx.restore();
  }
...
function tick(now){
  stepFly();
  for(let i=0; i<MAX_PARTICLES; i++) { const p = PARTICLE_POOL[i]; if(p.active) { p.x+=p.vx; p.y+=p.vy; p.vy+=.22; p.life-=.028; if(p.life<=0) p.active=false; } }
...
function localPt(e){ const rect=cv.getBoundingClientRect(); return [e.clientX-rect.left,e.clientY-rect.top]; }
cv.addEventListener('pointerdown',e=>{G.dragging=true;aimAt(...localPt(e));}); cv.addEventListener('pointermove',e=>{if(G.dragging)aimAt(...localPt(e));});
cv.addEventListener('pointerup',()=>{ if(!G.dragging)return; G.dragging=false; if(G.aim!=null)shoot(G.aim); G.aim=null; }); cv.addEventListener('pointercancel',()=>{G.dragging=false;G.aim=null;});

document.getElementById('btnSwap').onclick=()=>{ if(G.swaps<=0||G.fly||G.locked)return; SFX.click(); G.swaps--; const t=G.cur; G.cur=G.queue[0]; G.queue[0]=t; syncUI(); };
document.getElementById('btnHint').onclick=()=>{ if(G.hints<=0||G.fly||G.locked)return; SFX.click(); const hit=completionsFor(G.cur.s); if(!hit.length){toast('이 글자로는 만들 단어가 없어요');return;} hit.sort((a,b)=>(b.cat===G.goal)-(a.cat===G.goal)||b.word.length-a.word.length); G.hints--; G.hintCells=[[hit[0].c,hit[0].r]]; toast(hit[0].word,[[hit[0].c,hit[0].r]]); syncUI(); };
document.getElementById('btnBomb').onclick=()=>{ if(G.bombs<=0||G.fly||G.locked)return; SFX.click(); G.activeItem = G.activeItem==='bomb' ? null : 'bomb'; syncUI(); };
document.getElementById('btnRainbow').onclick=()=>{ if(G.rainbows<=0||G.fly||G.locked)return; SFX.click(); G.activeItem = G.activeItem==='rainbow' ? null : 'rainbow'; syncUI(); };
document.getElementById('btnMenu').onclick=()=>{
  if(veil.classList.contains('on'))return; G.locked=true;
  const info = `<p>주제: <b>${G.goal}</b> · 스테이지 ${G.stage}</p> <div>${G.targets.map(w=>`<span class="chip" style="${G.done[w]?'border-color:#7cffb2;color:#eafff3;text-shadow:0 0 8px #7cffb2':''}">${G.done[w]?'✓ ':''}${w}</span>`).join('')}</div>`;
...
document.getElementById('btnShop').onclick=()=>{ if(veil.classList.contains('on'))return; G.locked=true; SFX.click(); openShop(); };
...
function renderTargetBar(){
  ...
  if(G.mode==='theme'&&G.targets.length){
    ...
    bar.innerHTML=`<span class="target-label">🎯 목표단어</span>` + chips;
  }else if(G.mode==='free'){
    bar.style.display='flex';
    bar.innerHTML=`<span class="target-label">🎯 목표단어</span> <span class="tchip${G.wordsCompleted>=G.freeGoal?' done':''}">단어 ${G.wordsCompleted}/${G.freeGoal}개</span>`;
...
function syncUI(){
  ...
  document.getElementById('uiSwap').textContent=G.swaps; document.getElementById('uiHint').textContent=G.hints;
  renderTargetBar(); saveGame(false);
  document.getElementById('btnSwap').disabled=G.swaps<=0; document.getElementById('btnHint').disabled=G.hints<=0;
...
const veil=document.getElementById('veil'),card=document.getElementById('card'); function show(html){card.innerHTML=html;veil.classList.add('on');} function hide(){veil.classList.remove('on');}
...
function wireShop(){ card.querySelectorAll('button[data-id]').forEach(btn=>{ btn.onclick=()=>{ const it=SHOP_ITEMS.find(x=>x.id===btn.dataset.id); if(!it) return; const coins=SAVE.coins||0; if(coins<it.price) return; SAVE.coins=coins-it.price; it.apply(); saveGame(true); SFX.buy(); syncUI(); show(shopHTML()); wireShop(); }; }); document.getElementById('shopClose').onclick=()=>{ SFX.click(); hide(); G.locked=false; }; }
function calcStars(){ const goal = G.mode==='theme' ? G.targets.length : G.freeGoal; const idealShots = Math.max(goal, Math.ceil(goal*1.3)); const ratio = idealShots/Math.max(1, G.shots); if(ratio>=0.85) return 3; if(ratio>=0.55) return 2; return 1; }
...
function win(){
  G.locked=true;G.score+=1000; const stars=calcStars(); const isMilestone = G.mode==='theme' && G.stage%MILESTONE_EVERY===0; const isFinal = G.mode==='theme' && G.stage===MAX_STAGE; const milestoneBonus = isFinal?1000:(isMilestone?200:0); const coinGain = 30 + G.stage*4 + stars*15 + milestoneBonus; SAVE.coins=(SAVE.coins||0) + coinGain;
  ...
  let extra=''; if(G.mode==='theme'){ if(isFinal){ extra=`<p>🏆 100 스테이지를 모두 완주했어요!</p><p style="font-size:14px;opacity:.85">정말 대단해요. 계속해서 도전할 수 있어요.</p>`; }else if(isMilestone){ extra=`<p>🎁 마일스톤 달성! 보너스 코인 +${milestoneBonus}</p>`; }else{ const nextGoal=CATS[(G.stage)%CATS.length]; extra=`<p>목표 단어를 모두 만들었어요 🎉</p><p style="margin-top:8px;font-size:14px;opacity:.85">다음 주제: <b>${nextGoal}</b></p>`; } }else{ extra=`<p>목표 단어 개수를 달성했어요 🎉</p>`; }
  const mapBtn = G.mode==='theme' ? `<a href="#" id="toMap" style="display:block;margin-top:10px;color:#d9a94a;font-size:14px">🗺️ 지도로 보기</a>` : ''; const nextBtnLabel = isFinal ? '한번 더 플레이' : '다음 스테이지';
  show(`<h2>스테이지 ${G.stage} 완료!</h2>${starRow(stars)}${extra}<p>점수 <b>${G.score.toLocaleString()}</b> · 💰+${coinGain}</p><button class="btn" id="go">${nextBtnLabel}</button>${mapBtn}`);
  document.getElementById('go').onclick=()=>{ SFX.click(); if(G.mode==='theme'){ G.stage=Math.min(G.stage+1, MAX_STAGE); } else{ G.stage++; } hide();G.locked=false;buildStage();saveGame(true); };
...
function reviveBoard(){ for(let i=0;i<2 && G.grid.length>0;i++) G.grid.pop(); BOARDLAYER=null; toast('❤️ 부활! 아래 두 줄이 사라졌어요'); checkState(); syncUI(); }
function lose(){
  ...
  if(canRevive){ document.getElementById('revive').onclick=()=>{ SFX.buy(); SAVE.revives--; saveGame(true); hide(); G.locked=false; reviveBoard(); }; } document.getElementById('go').onclick=()=>{if(!spendLife())return;hide();G.locked=false;G.score=0;buildStage();};
}
function intro(){
...
function renderMapLives(){ const el=document.getElementById('mapLives'); if(!el)return; const s=computeLives(); if(s.count>=MAX_LIVES){ el.innerHTML=`❤️ ${s.count}/${MAX_LIVES}`; return; } const sec=Math.ceil(secToNextLife()); const mm=Math.floor(sec/60), ss=sec%60; el.innerHTML=`❤️ ${s.count}/${MAX_LIVES} <small>${mm}:${String(ss).padStart(2,'0')}</small>`; }
...
function openMap(_isRetry){
  document.getElementById('mapVeil').classList.add('on'); const stars=SAVE.theme.levelStars||{}; let maxUnlocked=1; for(let i=1;i<=MAX_STAGE;i++){ if(stars[i]!=null) maxUnlocked=i+1; } maxUnlocked=Math.min(maxUnlocked, MAX_STAGE); const allCleared = maxUnlocked>=MAX_STAGE && stars[MAX_STAGE]!=null; const TOTAL=Math.min(MAX_STAGE, Math.max(maxUnlocked+6, 12)); const scrollEl=document.getElementById('mapScroll'); const SPACING=108; const TOPPAD=60, BOTPAD=60; const containerW=scrollEl.clientWidth||390; const curZone=zoneIdx(TOTAL);
  function seeded(lv){ const x=Math.sin(lv*12.9898)*43758.5453; return x-Math.floor(x); } function xPct(lv){ const wave=Math.sin(lv*0.9)*26; const jitter=(seeded(lv)-0.5)*10; return 50+wave+jitter; }
  const zoneH=[]; for(let z=0; z<=curZone; z++){ const key=ZONES[z].key; zoneH[z] = PATH_POINTS[key] ? containerW*PATH_IMG_ASPECT[key] : 100*SPACING; } const H = zoneH.reduce((a,b)=>a+b,0) + TOPPAD + BOTPAD; const zoneTop=[], zoneBot=[]; { let bot=H-BOTPAD; for(let z=0; z<=curZone; z++){ zoneBot[z]=bot; zoneTop[z]=bot-zoneH[z]; bot=zoneTop[z]; } }
  function nodePos(lv){ const z=zoneIdx(lv); const key=ZONES[z].key; const localLv=lv-z*100; if(PATH_POINTS[key]){ const raw=PATH_POINTS[key][localLv-1] || [50,50]; return { x: raw[0], y: zoneTop[z] + (raw[1]/100)*zoneH[z] }; }else{ const y = zoneBot[z] - (localLv-0.5)/100*zoneH[z]; return { x: xPct(lv), y }; } }
  let zonesHtml=''; for(let z=0; z<=curZone; z++){ const key=ZONES[z].key; if(key==='forest'){ FOREST_SUBIMAGES.forEach(sub=>{ const subTop = zoneTop[z] + sub.topFrac*zoneH[z]; const subH = sub.hFrac*zoneH[z]; zonesHtml += `<img src="${sub.file}" style="position:absolute;left:0;top:${subTop}px;width:100%;height:${subH}px;z-index:0;pointer-events:none">`; }); }else if(PATH_POINTS[key]){ zonesHtml += `<img src="${ZONES[z].img}" style="position:absolute;left:0;top:${zoneTop[z]}px;width:100%;height:${zoneH[z]}px;z-index:0;pointer-events:none">`; }else{ zonesHtml += `<div style="position:absolute;left:0;top:${zoneTop[z]}px;width:100%;height:${zoneH[z]}px;overflow:hidden;background-image:url('${ZONES[z].img}');background-repeat:repeat-y;background-size:100% auto;background-position:top center"></div>`; } }
  let nodesHtml=''; let pathPts=[]; for(let lv=1; lv<=TOTAL; lv++){ const done = stars[lv]!=null; const isNext = !done && lv===maxUnlocked; const locked = !done && !isNext; const isMilestone = lv%MILESTONE_EVERY===0; const isFinal = lv===MAX_STAGE; const cls = done?'done':(isNext?'next':'locked'); const extraCls = isFinal?' mfinal':(isMilestone?' mmilestone':''); const p=nodePos(lv); pathPts.push([p.x,p.y]); const starSpots=[[-10,-1],[0,-5],[10,-1]]; const starHtml = done ? starSpots.map((sp,i)=>`<span class="mstar" style="left:calc(50% + ${sp[0]}px);top:${sp[1]}px">${i<stars[lv]?'★':'<span style=\'opacity:.35\'>★</span>'}</span>`).join('') : ''; const icon = isFinal ? '👑' : (isMilestone ? '🎁' : lv); nodesHtml += `<div class="mnode ${cls}${extraCls}" data-lv="${lv}" style="left:${p.x}%;top:${p.y}px">${done?'<span class="mdone-halo"></span>':''}${locked?'<span class="mlock">🔒</span>':icon}${starHtml}</div>`; }
  let pathD=''; pathPts.forEach((p,i)=>{ const [x,y]=p; if(i===0) pathD+=`M${x},${y}`; else{ const [px,py]=pathPts[i-1]; const midY=(py+y)/2; pathD+=` C${px},${midY} ${x},${midY} ${x},${y}`; } }); const svg=`<svg viewBox="0 0 100 ${H}" preserveAspectRatio="none" style="position:absolute;left:0;top:0;width:100%;height:${H}px;z-index:1;pointer-events:none"><path d="${pathD}" fill="none" stroke="#fff3c4" stroke-width="0.7" stroke-linecap="round" stroke-dasharray="0.5 1.2" opacity="0.45" vector-effect="non-scaling-stroke"/></svg>`; let html = `<div id="mapInner" style="height:${H}px">`; if(allCleared){ html += `<div style="position:absolute;left:50%;top:${TOPPAD-40}px;transform:translateX(-50%);color:#f5e3ae;text-align:center;font-size:14px;padding:6px 16px;white-space:nowrap;z-index:3">🏆 100 스테이지 완주! 대단해요</div>`; } html += zonesHtml + svg + nodesHtml + '</div>'; scrollEl.innerHTML=html;
  if(!_isRetry){ requestAnimationFrame(()=>{ const freshW = scrollEl.clientWidth; if(Math.abs(freshW - containerW) > 2){ openMap(true); return; } }); } renderMapLives(); clearInterval(_mapLivesTimer); _mapLivesTimer=setInterval(renderMapLives,1000); const tabsEl=document.getElementById('seasonTabs'); if(tabsEl){ if(ZONES.length <= 1){ tabsEl.innerHTML=''; } else { let tabsHtml=''; for(let z=0; z<ZONES.length; z++){ const reachable = z<=curZone; tabsHtml += `<div class="stab ${reachable?'':'locked'}" data-z="${z}">S${z+1}</div>`; } tabsEl.innerHTML=tabsHtml; tabsEl.querySelectorAll('.stab').forEach(t=>{ t.onclick=()=>{ const z=+t.dataset.z; if(z>curZone) return; SFX.click(); const targetY=(zoneTop[z]+zoneBot[z])/2; scrollEl.scrollTo({top: Math.max(0, targetY - scrollEl.clientHeight/2), behavior:'smooth'}); }; }); } } requestAnimationFrame(()=>{ const nextEl=scrollEl.querySelector('.mnode.next')||scrollEl.querySelector('.mnode.done:last-of-type'); if(nextEl) nextEl.scrollIntoView({block:'center'}); }); scrollEl.querySelectorAll('.mnode').forEach(el=>{ el.onclick=()=>{ const lv=+el.dataset.lv; if(el.classList.contains('locked')) return; if(!spendLife()) return; SFX.click(); document.getElementById('mapVeil').classList.remove('on'); clearInterval(_mapLivesTimer); G.mode='theme'; startGame(false, lv); }; });
}
...
function loadSave(){ try{ const raw=localStorage.getItem(SAVE_KEY); if(!raw) return null; const d=JSON.parse(raw); if(!d.free) d.free={stage:1,score:0,bestScore:0,bestStage:1}; if(!d.theme) d.theme={stage:1,score:0,bestScore:0,bestStage:1}; if(typeof d.coins!=='number') d.coins=0; if(typeof d.revives!=='number') d.revives=0; if(typeof d.totalStars!=='number') d.totalStars=0; if(!d.theme.levelStars) d.theme.levelStars={}; if(!d.lives) d.lives={count:6,lastUpdate:Date.now()}; return d; }catch(e){ return null; } }
...
function computeLives(){ let s=SAVE.lives; if(!s){ s=SAVE.lives={count:MAX_LIVES,lastUpdate:Date.now()}; } if(s.count<MAX_LIVES){ const now=Date.now(); const elapsed=now-s.lastUpdate; const regen=Math.floor(elapsed/LIFE_REGEN_MS); if(regen>0){ s.count=Math.min(MAX_LIVES, s.count+regen); s.lastUpdate+=regen*LIFE_REGEN_MS; if(s.count>=MAX_LIVES) s.lastUpdate=now; } }else{ s.lastUpdate=Date.now(); } return s; }
function secToNextLife(){ const s=computeLives(); if(s.count>=MAX_LIVES) return 0; return Math.max(0, LIFE_REGEN_MS - (Date.now()-s.lastUpdate)) / 1000; }
function spendLife(){ const s=computeLives(); if(s.count<=0){ const sec=Math.ceil(secToNextLife()); const mm=Math.floor(sec/60), ss=sec%60; show(`<h2>하트가 없어요 ❤️</h2><p>다음 하트까지 <b>${mm}:${String(ss).padStart(2,'0')}</b></p><p style="font-size:13px;opacity:.75;margin-top:6px">1분마다 하트가 하나씩 채워져요 (최대 ${MAX_LIVES}개)</p><button class="btn" id="livesOk">확인</button>`); document.getElementById('livesOk').onclick=()=>{ SFX.click(); hide(); }; return false; } s.count--; saveGame(true); return true; }
...
function applyDebugZones(){ const targetMax=Math.max(1, MAX_STAGE-1); const ls=SAVE.theme.levelStars||(SAVE.theme.levelStars={}); for(let i=1;i<=targetMax;i++){ if(ls[i]==null) ls[i]=3; } }
function boot(){ syncMuteBtn(); resize(); try{ const params=new URLSearchParams(location.search); if(params.get('testmap')==='1') applyDebugZones(); }catch(e){} G.grid=[]; G.targets=[]; G.cur=null; G.queue=[]; G.locked=true; intro(); requestAnimationFrame(tick); loadAssets().then(()=>{ BOARDLAYER=null; }); }
function markFontsReady(){ FONTS_READY=true; SPR.clear(); BOARDLAYER=null; }
if(document.fonts&&document.fonts.ready){ boot(); const need=[document.fonts.load("800 20px 'Pretendard'"),document.fonts.load("700 20px 'Pretendard'")]; Promise.all(need).then(()=>document.fonts.ready).then(markFontsReady).catch(()=>{ setTimeout(markFontsReady,800); }); setTimeout(()=>{ if(!FONTS_READY) markFontsReady(); },2000); } else { window.addEventListener('load',()=>{ boot(); setTimeout(markFontsReady,600); }); }
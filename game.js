...
const PALETTE = [['#8fc98a','#4f8a52'],['#e8a6c2','#c96f95'],['#8fb8d8','#4f7fa8'],['#e8c27a','#b8863f'],['#e0dcd0','#b0a890'],['#b39ad6','#7c5ba8']];
const NCOL=6;
...
const COLS=7, FRAME=9, BG_FRAME={left:0.028, right:0.972, top:0.148};
...
function resize(){
  ...
  const stageEl=document.getElementById('gameWrapper');
  if(!stageEl) return;
...
function explodeAt(c,r){
  ...
  setTimeout(()=>{
    let n=0; for(const [cc,rr] of cells){ if(!G.grid[rr]||!G.grid[rr][cc])continue; const col=colorOf(G.grid[rr][cc])[0]; burst(cx(cc,rr),cy(rr),col); addWave(cx(cc,rr),cy(rr),col,R*3); G.grid[rr][cc]=null; n++; }
...
function settle(f){
  ...
  G.grid[r][c]={s,col:f.col,born:performance.now()}; addShake(1.5); addWave(cx(c,r),cy(r),colorOf(G.grid[r][c])[0],R*1.4); resolve(c,r);
}
...
function resolve(c,r){
  ...
  if(word){
    ...
    if(word.word in G.done && !G.done[word.word]) { G.done[word.word]=true; const tBar = document.getElementById('targetBar'); if(tBar) { const chips = tBar.querySelectorAll('.tchip'); chips.forEach(chip => { if(chip.textContent.replace('✓ ','').trim() === word.word) { chip.style.transform = 'scale(1.25)'; chip.style.boxShadow = '0 0 20px #7ba05b'; chip.style.zIndex = '10'; setTimeout(() => { chip.style.transform = ''; chip.style.boxShadow = ''; chip.style.zIndex = ''; }, 400); } }); } }
...
const ASSET_SRC={ cannon:'assets/cannon.png', gem0:'assets/gem_jade.png', gem1:'assets/gem_ruby.png', gem2:'assets/gem_turquoise.png', gem3:'assets/gem_amber.png', gem4:'assets/gem_amethyst.png', gem5:'assets/gem_onyx.png' };
...
let BOARDLAYER=null;
function drawBoard(){
  if(!BOARDLAYER){ const c=document.createElement('canvas'); c.width=Math.ceil(W*DPR); c.height=Math.ceil(H*DPR); const g2=c.getContext('2d'); g2.setTransform(DPR,0,0,DPR,0,0); const keep=ctx; ctx=g2; const x=BX-FRAME,y=BY-FRAME,w=BW+FRAME*2,h=BH+FRAME*2; ctx.save(); roundRect(x,y,w,h,24); ctx.fillStyle='rgba(20,12,6,.0)';ctx.fill(); ctx.restore(); ctx=keep; BOARDLAYER=c; }
  ctx.drawImage(BOARDLAYER,0,0,W,H);
}
function pipePos(){ const cx0=W/2, sy=G.shooterY, img=ASSETS.cannon; if(!img) return {x:cx0,y:sy}; const w=R*4.8, h=w*(img.height/img.width); return {x: cx0-w/2+w*0.52, y: sy+R*0.0+h*-0.05}; }
function drawShooter(now){
  const cx0=W/2, sy=G.shooterY, img=ASSETS.cannon, p=pipePos();
  if(img){ const w=R*4.8, h=w*(img.height/img.width); ctx.drawImage(img, cx0-w/2, sy, w, h); }
  if(!G.fly&&G.cur){ const bob=Math.sin(now/420)*R*.05; bubble(p.x,p.y+bob,R*.94,G.cur.s,G.cur.col); if(G.activeItem){ ctx.save(); ctx.font=`${R*.62}px sans-serif`; ctx.textAlign='center';ctx.textBaseline='middle'; ctx.fillText(G.activeItem==='bomb'?'💣':'🌈', p.x+R*0.78, p.y+bob-R*0.78); ctx.restore(); } }
}
function drawQueue(){
...
function renderTargetBar(){
  ...
  if(G.mode==='theme'&&G.targets.length){
    bar.style.display='flex'; const chips = G.targets.map(w=>`<span class="tchip${G.done[w]?' done':''}">${G.done[w]?'✓ ':''}${w}</span>`).join('');
    bar.innerHTML=`<span class="target-label">🎯 목표단어</span>` + chips;
  }else if(G.mode==='free'){
    bar.style.display='flex'; bar.innerHTML=`<span class="target-label">🎯 목표단어</span> <span class="tchip${G.wordsCompleted>=G.freeGoal?' done':''}">단어 ${G.wordsCompleted}/${G.freeGoal}개</span>`;
...
const ZONES=[ {key:'forest', img:'assets/bg_hanok.jpg'} ]; function zoneIdx(lv){ return Math.min(4, Math.floor((lv-1)/100)); }
